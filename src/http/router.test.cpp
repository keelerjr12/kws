...
namespace {
  ...
  constexpr std::array<std::string, 3> route_patterns = {"/", "/first", "/first/second"};
  ...
  TEST_P(RouterTest, SingleRouteDefinitionRouteNonexistentThrowsException) {
    ...
    router.RegisterRoute({KWS::HttpMethod::GET, "/dummy"}, []([[maybe_unused]] const KWS::HttpRequest& req) {
  ...
  TEST_P(RouterTest, SingleRouteDefinitionRouteExistsDoesNotThrowException) {
    ...
    router.RegisterRoute({KWS::HttpMethod::GET, RouterTest::GetParam()}, []([[maybe_unused]] const KWS::HttpRequest& req) {
        return KWS::HttpResponse{KWS::HttpStatusCode::OK, ""};
    });
  ...
  TEST_P(RouterTest, MultipleRouteDefinitionsRouteExistsDoesNotThrowException) {
    ...
    for (const auto& route : route_patterns) {
      router.RegisterRoute({KWS::HttpMethod::GET, route}, []([[maybe_unused]] const KWS::HttpRequest& req) {
          return KWS::HttpResponse{KWS::HttpStatusCode::OK, ""};
      });
    }
